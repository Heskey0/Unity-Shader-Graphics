Shader "Custom/Fur"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            [maxvertexcount(42)]
            void geom(triangle Attributes inputArr[3], inout TriangleStream<Varyings> stream)
            {
                [loop] for (float i = 0; i < _ShellAmount; ++i)
                {
                    [unroll] for (float j = 0; j < 3; ++j)
                    {
                        Attributes input = inputArr[j];
                        int index = i;
                        ...
                        // step 1: compute disturbance(wind)
                        ...
                        // step 2: compute normal and disturb normal
                        float3 shellDir = SafeNormalize(normalInput.normalWS + move + windMove);

                        // step 3: move position along shellDir
            ...
            float4 frag(Varyings i) : SV_TARGET
            {
                ...
                // o.xyz = _WeightEnvLight * env_result;
                // o.xyz += 5.0 * sss;
                
                // shadow
                o.xyz *= mainLight.shadowAttenuation;
                return o;
            }
